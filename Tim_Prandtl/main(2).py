...
tim_test = ['With Prandtl correction', 'Without Prandtl correction']
for i, result in enumerate(all_results):
    ax_induction.plot(result['r/R_loc'], result['a'], label=f"{tim_test[i]}, a", color=blue_color[i])
    ax_induction.plot(result['r/R_loc'], result['a_prime'], label=f"{tim_test[i]}, a'", color=red_color[i+1])
...
for i, result in enumerate(all_results):
    ax_phi.plot(result['r/R_loc'], np.degrees(result['phi']), label=f"{tim_test[i]}", color=blue_color[i])
