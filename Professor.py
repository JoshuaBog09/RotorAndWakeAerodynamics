...
data1 = pd.read_excel(airfoil)

polar_alpha = data1['Alfa'].to_numpy()
polar_cl = data1['Cl'].to_numpy()
polar_cd = data1['Cd'].to_numpy()
polar_cm = data1['Cm'].to_numpy()
...
def solveStreamtube(Uinf, r1_R, r2_R, rootradius_R, tipradius_R, Omega, Radius, NBlades, chord, twist, polar_alpha,
    ...
    for i in range(Niterations):
        ...
        fnorm, ftan, gamma, phi, alpha = loadBladeElement(Urotor, Utan, r_R, chord, twist, polar_alpha, polar_cl, polar_cd)
    ...
    return [a, aline, r_R, fnorm, ftan, gamma, phi, alpha]
...
fig1 = plt.figure(figsize=(12, 6))
plt.title('Phi and alpha')
plt.plot(results[:, 2], results[:, 6], 'r-', label=r'$\phi$')
